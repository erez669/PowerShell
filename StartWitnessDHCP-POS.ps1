...
Write-Host "Trying primary target server name: $primaryServerName" -ForegroundColor Green

$serviceName = "DHCPServer"
Write-Host "Target service name: $serviceName" -ForegroundColor Green

Write-Host "`nAttempting to connect to primary server..." -ForegroundColor Yellow
...
function Manage-DHCPService {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            # Check if service is stopped before attempting to start
            $currentState = ($serviceStatus | Select-String "STATE" | Out-String)
            if (-not ($currentState -match "RUNNING")) {
                Write-Host "`nAttempting to start DHCP Server service..." -ForegroundColor Yellow
                $scStartCmd = "sc.exe \\$serverName start $serviceName"
                Invoke-Expression $scStartCmd | Out-Null
...
# If primary server fails, try the fallback server
if (-not $serverConnected) {
    $fallbackServerName = "PLPOSSRV$serverNum"
    Write-Host "`n### Primary server unavailable. Trying fallback server: $fallbackServerName ###" -ForegroundColor Yellow
    
    $serverConnected = Manage-DHCPService -serverName $fallbackServerName -serviceName $serviceName
...
Write-Host "`nScript execution completed at $(Get-Date)" -ForegroundColor Yellow
exit 0